<?php
    ...
    switch ($tipo) {
        ...
        case 'Eliminar Cuenta':
            ...
            break;
        /* Estas son funciones para recuperar contraseña y enviar una contraseña provicional, NO USARLAS (aún)
        case '1':
            recupera();
            break;
        case '2':
    ...
    function inicioSesion($conexion) {
        ...
        $correo = $conexion->prepare("SELECT idUsuario, contrasena FROM usuario WHERE correo = BINARY ?;");
        ...
        if(password_verify($pass, $passen['contrasena'])) { //El usuario pone escribe bien la contraseña
            $_SESSION["usuario"] = $user;
            $_SESSION["idUsuario"] = $passen['idUsuario'];
    ...
    function registro($conexion) { //Aqui insertamos al nuevo usuario.
        $nombre = isset($_POST["Nombre"]) ? $_POST["Nombre"]: ""; //nombre
        $user = isset($_POST["usuario"]) ? $_POST["usuario"]: ""; //correo
        $pass = isset($_POST["pass"]) ? $_POST["pass"]: "";       //contraseña
        $confpass = isset($_POST["confpass"]) ? $_POST["confpass"]: "";//contraseña repetida
        ...
        if ($pass == $confpass) { // Este if valida que las 2 contraseña coincidan
            if (($data = mysqli_fetch_assoc($result)) == FALSE ) {  //Es if verifica si el correo es igual 
                $passen = password_hash($pass, PASSWORD_DEFAULT); //Algoritmo bcrypt de encriptación de php (contraseña,PASSWORD_DEFAULT)
                //  Aqui insertamos los datos
                $sql = "INSERT INTO usuario(nombre, correo, contrasena) 
                        VALUES ('$nombre', '$user', '$passen')";
                
                if(($result = $conexion->query($sql)) === FALSE) {
                    echo "Error: " . $sql . "<br>" . $conexion->error;
                }
                // La variable toastmensaje inicializa el toast 
    ...
    function eliminarCuenta($conexion) {
        ...
        $query = $conexion->prepare("SELECT idUsuario, contrasena FROM usuario WHERE correo = ?;");
        ...
        $contraseña_almacenada = $registro['contrasena'];
        ...
        //Eliminamos datos de las tabla nota
        $user = $registro['idUsuario'];

        $query = "DELETE FROM nota WHERE idUsuario = '$user'";
